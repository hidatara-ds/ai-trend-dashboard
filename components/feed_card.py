...
def render_feed_card(post: Dict[str, Any]):
    ...
    html = f'<div class="feed-card"><div class="feed-header"><div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;"><span class="platform-badge badge-{platform}">{platform.upper()}</span><span style="font-size: 0.7rem; font-weight: 600; padding: 2px 8px; border-radius: 12px; background: {c_bg}; color: {c_text}; border: 1px solid {c_border};">{c_label}</span><span class="feed-author">{author}</span><span style="color: #71717a; font-size: 0.75rem;">• {created_rel}</span></div><div style="font-size: 0.75rem; color: #10b981; font-weight: 600;">SCORE {trend_score}</div></div><div class="feed-text">{text}</div>{translation_html}<div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #27272a; margin-top: 0.6rem; padding-top: 0.5rem; font-size: 0.75rem; color: #a1a1aa;"><div style="display: flex; gap: 1rem;"><span>LIKES <strong>{likes}</strong></span><span>COMMENTS <strong>{comments}</strong></span><span>SHARES <strong>{shares}</strong></span><span>VIEWS <strong>{views}</strong></span></div><a href="{url}" target="_blank" style="color: #06b6d4; text-decoration: none; display: inline-flex; align-items: center; gap: 4px;">Original Post {ext_icon}</a></div></div>'
